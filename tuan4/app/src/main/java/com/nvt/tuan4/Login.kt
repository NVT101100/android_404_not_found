...
import com.nvt.tuan4.databinding.LoginBinding
...
class Login : AppCompatActivity() {
    ...
    private fun onSubmit() {
        ...
        if(myModel.login(email,password)){
            val listRes:Intent = Intent(this,ListRestaurant::class.java)
            //listRes.putExtra("user",User(DataStore.name,DataStore.email,DataStore.phone))
            startActivity(listRes)
            finish()
        }
